...
class drive_command_node():
	...
	def obstacle_cb(self, pos_list):
		...
		if (pos_list.size == 4):
			...
			self.entrance_stop_x = entrance_x - (exit_x - entrance_x) / 7
			self.entrance_stop_y = entrance_y - (exit_y - entrance_y) / 7
			self.exit_stop_x = exit_x + (exit_x - entrance_x) / 7
			self.exit_stop_y = exit_y + (exit_y - entrance_y) / 7
